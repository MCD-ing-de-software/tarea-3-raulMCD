...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        self.assertEqual(result.shape, esperado.shape,
                         "El valor de resultado debería tener la forma esperada.")
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError):
            result = utils.moving_average(arr, window=len(arr)+1)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        self.assertAlmostEqual(result.mean(), 0.0, places = 10, msg="La media del resultado debería ser aproximadamente 0.")
        self.assertAlmostEqual(result.std(), 1.0, places=10,
                               msg="La desviación estándar del resultado debería ser aproximadamente 1.")
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        self.assertAlmostEqual(result.min(), 0.0, places=10,
                               msg="El valor mínimo del resultado debería ser aproximadamente 0.0")
        self.assertAlmostEqual(result.max(), 1.0, places=10,
                               msg="El valor mínimo del resultado debería ser aproximadamente 1.0")
        npt.assert_allclose(result, esperado, rtol=1e-7, atol=1e-7)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        helpers = StatisticsUtils()
        arr = [3, 3, 3]
        with self.assertRaises(ValueError):
            result = helpers.min_max_scale(arr)
